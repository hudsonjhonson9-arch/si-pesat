...
export default function NotaDinasGenerator({ audit, activeCategory, userProfiles = [], onClose }: NotaDinasGeneratorProps) {
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      const opt = {
        margin:       0,
        filename:     `Nota_Dinas_${audit.opdName.replace(/\s+/g, '_')}.pdf`,
                ...
                <div className="space-y-2 bg-white p-3 rounded-xl border border-slate-200">
                  {teamList.map((member, i) => (
                    <div key={member.id} className="flex gap-2">
                      <input type="text" placeholder="Nama" value={member.nama} onChange={e => handleUpdateTeam(member.id, 'nama', e.target.value)} className="w-2/3 text-xs p-2 border border-slate-200 rounded outline-none focus:border-emerald-500" />
                      <input type="text" placeholder="Jabatan" value={member.jabatan} onChange={e => handleUpdateTeam(member.id, 'jabatan', e.target.value)} className="w-1/3 text-xs p-2 border border-slate-200 rounded outline-none focus:border-emerald-500" />
                    </div>
                  ))}
                </div>
              </div>
