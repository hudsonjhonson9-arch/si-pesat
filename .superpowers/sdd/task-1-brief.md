...
- Modify: `supabase_schema.sql` (append migration content)

**Goal:** Create all new database tables (bidang, roles, permissions, role_permissions), add bidang_id FK columns to existing tables, seed default data, and update role constraint.

## Requirements

1. Create `bidang` table with 5 rows (Irban I—V, Irban IV gets 'Kecamatan Loli' as wilayah)
2. Create `roles` table with 15 roles (13 existing + Sekretaris + PPPK)
3. Create `permissions` table with 14 permission codes
4. Create `role_permissions` table with seed mappings:
   - Inspektur (id=13): all permissions, scope='all'
   - Sekretaris (id=14): all permissions, scope='all'
   - Inspektur Pembantu (id=12): all permissions, scope='bidang'
   - Auditor Ahli Muda/Madya/Utama (5-7): audit.view, audit.create, audit.edit, evidence.upload, entity.view, stats.view — scope='bidang'
   - Auditor Ahli Pertama (4): audit.view, audit.edit, evidence.upload, entity.view, stats.view — scope='bidang'
   - Auditor Penyelia/Lanjutan/Pelaksana (1-3): audit.view, evidence.upload, entity.view, stats.view — scope='bidang'
   - PPUPD semua level (8-11): audit.view, evidence.upload, entity.view, stats.view — scope='bidang'
   - PPPK (15): audit.view, evidence.upload — scope='bidang'
5. Add `bidang_id INT REFERENCES bidang(id)` to: profiles, audits, target_entities, templates
6. Set existing data to bidang_id = 4 (Irban IV)
7. Update profiles role CHECK constraint to include 'Sekretaris' and 'PPPK'
8. Append to `supabase_schema.sql` so fresh installs get the new schema

## Exact SQL

Use this exact SQL for the migration file:
...
1. Read the migration file back to confirm it exists and has correct content
2. Check that `supabase_schema.sql` has the migration appended

## No tests

This is SQL only — no test framework. Verification is by file inspection.
